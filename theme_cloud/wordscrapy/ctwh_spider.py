...
class CtwhSpider:  # 传统文化网站爬取文章信息
    ...
    def get_url_list(self):
        # 生成爬取文章url列表
        return [self.url_temp.format(i) for i in range(28, 40)]
    ...
    # def save_mysql(self,title,data,path):
    #     conn = pymysql.connect('127.0.1','root','password','root',charset = "utf-8")
    #     cursor = conn.cursor()
    #     # 数据库操作
    #     # (1)定义一个格式化的sql语句
    #     sql = 'insert into qiushi(title,data,path) values(%s,%s,%s) '
    #     # (2)准备数据
